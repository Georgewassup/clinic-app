...
export default function ManageProduct() {
  ...
  return (
    <ModuleLayout title={t('home.modules.product')}>
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-[24px] font-bold text-[#1c1c1e]">{t('home.modules.product')}</h2>
        <Button>+ Add Product</Button>
      </div>
      {products.length === 0 ? (
        <EmptyState title="No products" message="Add your first product to the catalog." action={<Button>+ Add Product</Button>} />
      ) : (
            ...
            <table className="w-full text-[15px]">
              <thead>
                <tr className="bg-[#f8f8fc] border-b border-[#c6c6c8]/20">
                  ...
                  <th className="text-left py-3.5 px-4 text-[13px] font-bold text-[#8e8e93] uppercase tracking-wider">Price</th>
