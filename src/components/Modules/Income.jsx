...
export default function Income() {
  ...
  const { data: incomes } = useFirestore(transactionService);

  return (
    <ModuleLayout title={t('home.modules.income')}>
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-[24px] font-bold text-[#1c1c1e]">{t('home.modules.income')}</h2>
        <Button>+ Add Income</Button>
      </div>
      {incomes.length === 0 ? (
        <EmptyState title="No income records" message="Track your clinic's income here." action={<Button>+ Add Income</Button>} />
      ) : (
